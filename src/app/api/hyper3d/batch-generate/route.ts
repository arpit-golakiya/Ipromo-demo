...
function resolveItemImageUrls(item: BatchItemInput): string[] {
...
async function startTaskWithRetry(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    try {
      return await startRodinTask({ imageUrls, removeLogosFor3D });
    } catch (err) {
...
export async function POST(req: NextRequest) {
  ...
  try {
    const results = await runWithConcurrency(items, 3, async (item): Promise<BatchItemResult> => {
      ...
      try {
        const taskId = await startTaskWithRetry(imageUrls, body.removeLogosFor3D === true);
        return {
          key: item.key,
