...
export default function EditBrandPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        {
          const base = json.brand.logoVariants.length ? json.brand.logoVariants : [json.brand.imageUrl];
          setVariants(base);
          setGeneratedVariants(base);
        }
  ...
  async function onApprove() {
    ...
    setError(null);
    try {
      const baseVariants = original.logoVariants.length ? original.logoVariants : [original.imageUrl];
      const currentVariants = variants ?? baseVariants;

      const variantsChanged =
        currentVariants.length !== baseVariants.length ||
        currentVariants.some((u, i) => u !== baseVariants[i]) ||
        currentVariants.some((u) => u.startsWith("data:"));

      // If already approved and nothing changed, just redirect.
      if (original.isApproved && !variantsChanged) {
      ...
      if (!original.isApproved) body.isApproved = true;
      ...
      if (!res.ok) {
        const json: unknown = await res.json().catch(() => ({}));
        throw new Error((json as { error?: string } | null)?.error ?? "Failed to save brand");
      }
      router.replace("/brands");
      router.refresh();
    } catch (e) {
              ...
              <div className="grid gap-4">
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  {[
                    ...
                    const base = generatedVariants?.[card.idx] ?? original.logoVariants?.[card.idx] ?? original.imageUrl;
                    return (
